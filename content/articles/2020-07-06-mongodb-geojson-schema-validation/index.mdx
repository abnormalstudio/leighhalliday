...
This schema validation requires that the `location` field exists, that it is an object with a `type` field with an enum value of `Point`, and a `coordinates` field of two numbers between -180 and 180. Keep in mind that latitudes should be restricted between -90 and 90, but I am not sure how to ensure that the 2nd number of an array has a different range of values from the first one... if anyone knows, let me know 😜!
