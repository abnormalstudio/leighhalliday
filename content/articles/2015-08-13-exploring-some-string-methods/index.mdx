...
`gsub` is a great method which looks for a pattern (regular expression) and replaces __ALL__ instances of that pattern with your replacement string. This differs from the `sub` method which only replaces the first instance found. 
